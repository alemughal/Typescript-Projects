...
async function bankService(bank) {
    do {
        ...
        if (service.select === "Deposit") {
            ...
            let account2 = bank.account.find((acc) => acc.accNumber === accountNumber2);
            if (!account2) {
                console.log(chalk.bold.red("Account Not Found"));
            }
            let ans = await inquirer.prompt({
            ...
            if (account && account2) {
                {
                    ...
                    if (newBalance < 0) {
                        ...
                        console.log(chalk.blue(`You have Transferred ${ans.amount} to Account: ${account2.accNumber}`));
                        console.log(chalk.green(`New Balance: ${newBalance}`));
                    }
                }
            }
            ...
            if (!account) {
                ...
                let name = bank.customer.find((item) => item.accNumber === accountNumber);
                console.log(`Dear ${chalk.green.italic(name?.firstName)} ${chalk.green.italic(name?.lastName)} Your Account ${chalk.green.italic(account.accNumber)} Balance is ${chalk.blueBright.bold("$" + account.balance)}`);
            }
        }
